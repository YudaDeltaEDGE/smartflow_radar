...
from steps.F_sell_lot.step import jalankan_step_F_sell_lot
...
def main() -> None:
    ...
    print(f"Total total_sell_lot_5mnt: {len(ctx.total_sell_lot_5mnt)}")
    ...
    ctx = jalankan_step_F_sell_lot(ctx)
    print(f"CSV sell lot tersimpan: {ctx.csv_sell_lot_5mnt_path}")

    sample2 = list(sorted(ctx.total_sell_lot_5mnt_values.items()))[:10]
    print("\n=== SAMPLE total_sell_lot_5mnt_values (10) ===")
    for em, v in sample2:
        print(f"{em}: {v}")
